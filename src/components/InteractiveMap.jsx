                ...
                <div className="space-y-2">
                  {Object.entries(current.categories).map(([key, label]) => (
                    <Button
                      key={key}
                      variant={
                        visibleCategories.includes(key) ? "default" : "outline"
                      }
                      size="sm"
                ...
                <div
                  ...
                  {filteredLocations.map((location, index) => {
                    ...
                    return (
                      <div
                        key={index}
                        className={`absolute w-8 h-8 ${getTypeColor(
                          location.type
                        )} rounded-full flex items-center justify-center text-white cursor-pointer hover:scale-125 transition-all duration-200 ${
                        ...
                        onClick={() =>
                          setSelectedLocation(
                            selectedLocation === index ? null : index
                          )
                        }
                        title={location.name}>
                ...
                <div className="flex items-center gap-4 mb-4">
                  <div
                    className={`w-12 h-12 ${getTypeColor(
                      filteredLocations[selectedLocation].type
                    )} rounded-full flex items-center justify-center`}>
                    {(() => {
                      const IconComponent =
                        filteredLocations[selectedLocation].icon;
